    ...
    if( create ) {
        ...
        if( connect(config->sockets[OUT_SOCKET], (struct sockaddr *)&addr_in, sizeof(struct sockaddr_in)) < 0 ) {
            if( errno != EINPROGRESS ) {
                // non-blocking socket, connection not in progress (error)
...
static void process_terminal(ST_FORWARDER *config, char *bufer, ssize_t size)
{
    ...
    if( msg->encode ) {
        // encode need, data = ST_RECORD*, msg->len = number of the records in data
        ...
        */
        ...
        // data = raw terminal data, msg->len = data size
                    ...
                    if( bytes_read > 0 ) {
                        ...
                        if( log_server_answer ){
                            ...
                            if( strstr(config->name, "egts") ){
                                ...
                                if( *(uint16_t*)&config->buffers[OUT_RDBUF][14] == log_server_answer ){
                                    // EGTS_SR_RECORD_RESPONSE.CRN
