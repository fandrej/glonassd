    ...
    while( parcel_pointer < pak_head->HL + pak_head->FDL ) {
        ...
        if( !rec_head->RL ) {
            ...
            if( answer->count || answer->size ) {
                // успели расшифровать несколько записей
        ...
        while(sdr_readed < rec_head->RL) {
            ...
            case EGTS_SR_TERM_IDENTITY:    // авторизация
                ...
                if( !Parse_EGTS_SR_TERM_IDENTITY( (EGTS_SR_TERM_IDENTITY_RECORD *)&parcel[parcel_pointer], answer, worker ) && !st_rec_header.OID ) {
                    // нет ID терминала
...
int Parse_EGTS_RECORD_HEADER(EGTS_RECORD_HEADER *rec_head, EGTS_RECORD_HEADER *st_header, ST_ANSWER *answer, ST_WORKER *worker)
{
    ...
    */
    if( st_header->RFL & B0 ) {
        ...
        if( st_header->OID /*&& !strlen(answer->lastpoint.imei)*/ ) {
            // D:\Work\Borland\Satellite\egts\Рекомендации по реализации протокола передачи данных в РНИЦ.doc 5.1.1    Идентификация АС посредством поля OID
...
int Parse_EGTS_SR_TERM_IDENTITY(EGTS_SR_TERM_IDENTITY_RECORD *record, ST_ANSWER *answer, ST_WORKER *worker)
{
    ...
    */
    if( (record->FLG & B1) ) {
        // наличие поля IMEI в подзаписи
        // пропускаем поле HDID, если есть
        if( record->FLG & B0 ) {
            // наличие поля HDID в подзаписи
    ...
    /*
       if( record->FLG & B2 ){
           // наличие поля IMSI в подзаписи
        record_size += EGTS_IMSI_LEN;
       }

       if( record->FLG & B3 ){
           // наличие поля LNGC в подзаписи
        record_size += EGTS_LNGC_LEN;
       }

       if( record->FLG & B5 ){
           // наличие поля NID в подзаписи
        record_size += (3 * sizeof(uint8_t));
       }

       if( record->FLG & B6 ){
           // наличие поля BS в подзаписи
        record_size += sizeof(uint16_t);
       }

       if( record->FLG & B7 ){
           // наличие поля MSISDN в подзаписи
...
int Parse_EGTS_SR_POS_DATA(EGTS_SR_POS_DATA_RECORD *posdata, ST_RECORD *record, ST_ANSWER *answer, ST_WORKER *worker)
{
    ...
    /* присылаются в WGS84
       if( posdata->FLG & B1 ){
           // прислано в ПЗ-90.02, надо перевести в WGS-84 ибо Если координаты не трансформировать, то возникнет погрешность до 10 м
...
int Parse_EGTS_SR_EXT_POS_DATA(EGTS_SR_EXT_POS_DATA_RECORD *posdata, ST_RECORD *record)
{
    ...
    if( posdata->FLG & B3 ) {
        // Satellites Field Exists
        record->satellites = *(uint8_t *)&pc[data_size];
        data_size += sizeof(uint8_t);
    }

    if( posdata->FLG & B4 ) {
        // Navigation System Field Exists
        data_size += sizeof(uint16_t);
    }
...
int Parse_EGTS_SR_LIQUID_LEVEL_SENSOR(int rlen, EGTS_SR_LIQUID_LEVEL_SENSOR_RECORD *posdata, ST_RECORD *record)
{
    ...
    if( posdata->FLG & B3 ) {
        ...
        if( !(posdata->FLG & B6) ) {
            ...
            if(num < 4) {
                // у нас только 4 бака предусмотрено
...
int Parse_EGTS_SR_COMMAND_DATA(ST_RECORD *record, ST_ANSWER *answer, EGTS_SR_COMMAND_DATA_RECORD *command, uint16_t command_len, ST_WORKER *worker)
{
    ...
    if( command->ACFE & 1 ){
        // поле CHS присутствует в подзаписи
        CHS = (uint8_t *)CD;
        CD += sizeof(uint8_t);
    }

    if( command->ACFE & 2 ) {
        // поля ACL и AC присутствуют в подзаписи
        ACL = (uint8_t *)CD;
