...
int terminal_encode(ST_RECORD *records, int reccount, char *buffer, int bufsize)
{
    ...
    struct {
        uint8_t     header;     // 0x01
        uint16_t    len_rec;    // packet length
        // data:
        // IMEI
        uint8_t     tag_imei;   // 0x03
        char        imei[15];
        // date & time
        uint8_t     tag_date;   // 0x20
        uint32_t    date;       // UNIX timestamp
        // coordinates + valid + satellites
        uint8_t     tag_position;   // 0x30
        uint8_t     position_sat_val;   //
        int32_t     position_lat;
        int32_t     position_lon;
        // speed + curse
        uint8_t     tag_speed;  // 0x33
        uint16_t    speed_speed;
        uint16_t    speed_curse;
        // altitude
        uint8_t     tag_altitude;  // 0x34
        int16_t     altitude;
        // status
        uint8_t     tag_status;  // 0x40
        uint16_t    status;
        // end data
        uint16_t    crc16;
    } parcel;
    ...
    parcel.tag_date =       0x20;
    parcel.tag_position =   0x30;
    parcel.tag_speed =      0x33;
    parcel.tag_altitude =   0x34;
    parcel.tag_altitude =   0x40;
    parcel.len_rec = sizeof(parcel) - (sizeof(uint16_t) + sizeof(uint8_t) + sizeof(uint16_t));
    ...
    for(i = 0; i < reccount; i++) {
        ...
        parcel.position_lat = (int32_t)(1000000 * records[i].lat);
        parcel.position_lon = (int32_t)(1000000 * records[i].lon);
        parcel.speed_speed = (uint16_t)(10 * records[i].speed);
        parcel.speed_curse = (uint16_t)(10 * records[i].curs);
        parcel.altitude = (int16_t)records[i].height;
        parcel.status = (uint16_t)records[i].status;
        parcel.crc16 = CRC16((unsigned char *)&parcel, (unsigned short)sizeof(parcel) - sizeof(uint16_t));
