...
#pragma pack( push, 1 )
typedef struct {
    uint16_t	ADR;	// адрес модуля, для которого данная команда предназначена
    uint8_t		SZ_ACT;	// объём памяти для параметра (используется совместно с действием ACT=3), описание действия
    uint16_t	CCD;	// код команды из Таблицы 35 при ACT=0 или параметра из Таблицы 35 при ACT=1..4
    /* необязательные поля
...
typedef struct {
    ...
    uint32_t	CID;		// (Command Identifier) идентификатор команды, сообщения.
    uint32_t	SID;		// (Source Identifier) идентификатор отправителя (уровня прикладного ПО)
    uint8_t		ACFE;		//  Flags
    /* необязательные поля
    ...
    uint8_t		ACL;		// (Authorization Code Length) длина в байтах поля AC
    uint8_t		AC[ACL];	// (Authorization Code) код авторизации, использующийся на принимающей стороне (АТ)
...
CT :7654    - Command Type:
    ...
    0100 = (64) = CT_MSGTO - информационное сообщение для вывода на устройство отображения АТ
	0011 = (48) = CT_MSGFROM - информационное сообщение от АТ
...
*/
#define CT_DELIV   (128)
#define CT_SUBREQ  (112)
#define CT_DELCOM  (96)
#define CT_COM     (80)
#define CT_MSGTO   (64)
#define CT_MSGFROM (48)
#define CT_MSGCONF (32)
#define CT_COMCONF (16)
/*
CCT:3210 - Command Confirmation Type:
